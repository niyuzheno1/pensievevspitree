...
DTModel = "Model/pensieve_fcc_100.pk3"
...
# NN_MODEL = None
NN_MODEL = "../rl_server/models/myinfo2100.ckpt"
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                try:
                    state.append(VIDEO_BIT_RATE[post_data['lastquality']] / float(np.max(VIDEO_BIT_RATE)))
                    state.append( post_data['buffer'] / BUFFER_NORM_FACTOR)  
                    state.append(float(video_chunk_size) / float(video_chunk_fetch_time) / M_IN_K )
                    state.append(float(video_chunk_fetch_time) / M_IN_K / BUFFER_NORM_FACTOR)
                ...
                action2 = self.network.choose_action(state2)
...
def run(server_class=HTTPServer, port=8333, log_file_path=LOG_FILE):
    ...
    if True:
        ...
        video_chunk_count = 0
        network = a2c.A2CNetwork()
        nn_model = NN_MODEL
        saver = tf.train.Saver()
        if nn_model is not None:  # nn_model is the path to file
                saver.restore(network.session, nn_model)
        print("Model restored.")
        input_dict = {'train_counter': train_counter,
...
def main():
    if len(sys.argv) == 2:
        ...
        run(log_file_path=LOG_FILE + '_A2C_' + trace_file)
    else:
        run()
