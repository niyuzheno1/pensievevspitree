        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                self.log_file.write(str(time.time()) + '\t' +
                                    ...
                                    str(reward) + '\n')
                self.log_file.flush()
                serialized_state = self.serial(state)
                bit_rate = int(self.policy.predict(np.array(serialized_state).reshape(1, -1))[0])
...
def run(server_class=HTTPServer, port=8333, log_file_path=LOG_FILE):
    ...
    with open(log_file_path, 'wb') as log_file:
        ...
        input_dict = {'train_counter': train_counter,'log_file': log_file,
...
def main():
    if len(sys.argv) == 2:
        ...
        run(log_file_path=LOG_FILE + '_PiTree_' + trace_file)
    else:
        run()
