...
SCHEMES = ['sim_rl', 'sim_dt' , 'sim_myrl2']
M_IN_K = 1000.0
REBUF_PENALTY = 4.3
SMOOTH_PENALTY = 1
...
def main():
    ...
    df = pd.DataFrame({'metric': ['Qlin', 'Qlog', 'QHD'],
                   ...
                   ,'Reward (A2C)': [sum_rewards_2[ 'sim_myrl2' + '|:|' + 'lin' ]/sum_rewards_2[ 'sim_rl' + '|:|' + 'lin' ], sum_rewards_2[ 'sim_myrl2' + '|:|' + 'log' ]/ sum_rewards_2[ 'sim_rl' + '|:|' + 'log' ], sum_rewards_2[ 'sim_myrl2' + '|:|' + 'hd' ]/ sum_rewards_2[ 'sim_rl' + '|:|' + 'hd' ]]
    })
    df = df[['metric','Reward (PiTree)','Reward (Pensieve)','Reward (A2C)']]
    ...
    df = pd.DataFrame({'metric': ['average buffer size', 'average bandwidth', 'average delay'],
                   ...
                   'measurement (A2C)': [np.average(buffer_size_avg['sim_myrl2'])/a[0], np.average(bandwidth_avg['sim_myrl2'])/a[1], np.average(delay_avg['sim_myrl2'])/a[2]  ]
    })
    df = df[['metric','measurement (PiTree)','measurement (Pensieve)','measurement (A2C)']]
